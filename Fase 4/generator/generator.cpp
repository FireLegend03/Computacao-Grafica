...
std::string generateTorus(float outerRadius, float innerRadius, int rings, int stacks) {
    ...
    for (int i = 0; i < rings; i++) {
        for (int j = 0; j < stacks; j++) {
            // Ângulos atuais
            float theta = i * outerAngle;
            float nextTheta = (i + 1) * outerAngle;
            float phi = j * innerAngle;
            float nextPhi = (j + 1) * innerAngle;

            // Ponto 1
            float x1 = (outerRadius + innerRadius * cos(phi)) * cos(theta);
            float y1 = innerRadius * sin(phi);
            float z1 = (outerRadius + innerRadius * cos(phi)) * sin(theta);

            // Ponto 2
            float x2 = (outerRadius + innerRadius * cos(nextPhi)) * cos(theta);
            float y2 = innerRadius * sin(nextPhi);
            float z2 = (outerRadius + innerRadius * cos(nextPhi)) * sin(theta);

            // Ponto 3
            float x3 = (outerRadius + innerRadius * cos(phi)) * cos(nextTheta);
            float y3 = innerRadius * sin(phi);
            float z3 = (outerRadius + innerRadius * cos(phi)) * sin(nextTheta);

            // Ponto 4
            float x4 = (outerRadius + innerRadius * cos(nextPhi)) * cos(nextTheta);
            float y4 = innerRadius * sin(nextPhi);
            float z4 = (outerRadius + innerRadius * cos(nextPhi)) * sin(nextTheta);

            // Normais (do centro do tubo até o ponto na superfície)
            auto normalize = [](float x, float y, float z) {
                float len = sqrt(x * x + y * y + z * z);
                return std::tuple<float, float, float>(x / len, y / len, z / len);
                };
            ...
            float uNext = (float)(i + 1) / rings;
            float v = (float)j / stacks;
            float vNext = (float)(j + 1) / stacks;

            // Triângulo 1: 
            triangles << "v " << x1 << " " << y1 << " " << z1 << "\n";
            std::tie(nx, ny, nz) = normalize(x1 - outerRadius * cos(theta), y1, z1 - outerRadius * sin(theta));
            triangles << "n " << nx << " " << ny << " " << nz << "\n";
            triangles << "t " << u << " " << v << "\n";

            triangles << "v " << x2 << " " << y2 << " " << z2 << "\n";
            std::tie(nx, ny, nz) = normalize(x2 - outerRadius * cos(theta), y2, z2 - outerRadius * sin(theta));
            triangles << "n " << nx << " " << ny << " " << nz << "\n";
            triangles << "t " << u << " " << vNext << "\n";

            triangles << "v " << x4 << " " << y4 << " " << z4 << "\n";
            std::tie(nx, ny, nz) = normalize(x4 - outerRadius * cos(nextTheta), y4, z4 - outerRadius * sin(nextTheta));
            triangles << "n " << nx << " " << ny << " " << nz << "\n";
            triangles << "t " << uNext << " " << vNext << "\n";
            ...
            std::tie(nx, ny, nz) = normalize(x3 - outerRadius * cos(nextTheta), y3, z3 - outerRadius * sin(nextTheta));
            triangles << "n " << nx << " " << ny << " " << nz << "\n";
            triangles << "t " << uNext << " " << v << "\n";
